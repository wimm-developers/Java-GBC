...
/**
 * This class can mix a square wave signal with a sound buffer. It supports all
 * features of the Gameboys sound channels 1 and 2.
 */
class SquareWaveGenerator {
	...
	/**
	 * If true, envelope will increase amplitude of sound, false indicates
	 * decrease
	 */
	...
	/**
	 * If true, sweep will decrease the sound frequency, otherwise, it will
	 * increase
	 */
	...
	/**
	 * Output a frame of sound data into the buffer using the supplied frame
	 * length and array offset.
	 */
	public void play(byte[] b, int length, int offset) {
		...
		if (totalLength != 0) {
			...
			for (int r = offset; r < end; r++) {
				...
				/*
				 * if (cyclePos >= (cycleLength / 2)) { val = amplitude; } else
				 * { val = -amplitude; }
				 */
