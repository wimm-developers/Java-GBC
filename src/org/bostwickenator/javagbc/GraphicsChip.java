...
public abstract class GraphicsChip {
	...
	private final int maxFrameSkip = 4;
	...
	protected int transparentCutoff; // min "attrib" value where transparency
	// can occur
	...
	protected int colorCount; // number of "logical" colors = palette indices,
	// 12 for gb, 64 for gbc
	...
	public int unflatten(byte[] flatState, int offset) {
		...
		for (int i = 0; i < 12; i++) {
			if ((i & 3) == 0)
				gbPalette[i] = 0x00ffffff & Dmgcpu.getInt(flatState, offset);
			else
				gbPalette[i] = 0xff000000 | Dmgcpu.getInt(flatState, offset);
			offset += 4;
		}
	...
	public int flatten(byte[] flatState, int offset) {
		...
		for (int j = 0; j < 12; j++) {
			Dmgcpu.setInt(flatState, offset, gbPalette[j]);
			offset += 4;
		}
	...
	public final void vBlank() {
		...
		if (skipping) {
			...
			if (skipCount >= maxFrameSkip) {
				// can't keep up, force draw next frame and reset timer (if
				// lagging)
				skipping = false;
	...
	public void setGBCPalette(int index, int data) {
		...
		gbcPalette[index >> 1] = gbcMask + ((value & 0x001F) << 19)
				+ ((value & 0x03E0) << 6) + ((value & 0x7C00) >> 7);
	...
	public abstract int[] setScale(int screenWidth, int screenHeight);
